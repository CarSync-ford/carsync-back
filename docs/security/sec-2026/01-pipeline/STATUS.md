...
Resultado observado e data: Pipeline integrado configurado com 5 gates obrigatórios (`needs: [test, sast, sca, secret-scan, container-scan]`). Deploy restrito a push na main, sem rebuild de JAR/Docker, baixando artefato da imagem com `actions/download-artifact` fixada por SHA verificado (`d3f86a106a0bac45b974a628896c90dbdf5c8093 # v4.3.0`) e validando ID do container antes de qualquer credencial ACR/Azure. Digest de publicação capturado estritamente e persistido em `deploy-evidence/published-digest.txt`. Separação estrita entre verificação de PR e deploy em nuvem: a especificação e o aceite da frente não exigem execução de deploy cloud real. O PR #33 executou testes, SAST, SCA e secrets com sucesso (run 36074219523) com deploy ignorado. Execução remota do novo job `container-scan` aguarda trigger no Actions; ausência de deploy real e de digest no registry mantida explícita; 2026-09-25.
Evidência: `.github/workflows/deploy.yml`; run 36074219523; `check-container.sh`; `REPORT.md`.
Dependência externa / responsável / ação para desbloquear: GitHub Actions / mantenedor / autorizar execução do novo workflow no Actions para validação do `container-scan`.
...
Resultado observado e data: nenhum diff no Dockerfile entre a base e o estado final desta branch (`sec-2026/01-pipeline`); re-scan não aplicável antes da consolidação; 2026-09-25.
Evidência: comparação Git sem saída; base `41d2227`.
